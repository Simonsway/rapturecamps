...
module ModelAutoCompleterHelper
  ...
  def model_auto_completer_result(models, display, phrase=nil)
    ...
    render :inline => '<ul>' + items.uniq.join + '</ul>'
  end
  ...
  def model_auto_completer(tf_name, tf_value, hf_name, hf_value, options={}, tag_options={}, completion_options={})
    ...
    html = ""
    html << auto_complete_stylesheet unless completion_options[:skip_style]
    html << hidden_field_tag(hf_name, hf_value, :id => hf_id)
    html << text_field_tag(tf_name, tf_value, tag_options)
    html << content_tag("div", "", :id => "#{tf_id}_auto_complete", :class => "auto_complete")
    html << auto_complete_field(tf_id, completion_options)
    render :inline => html
  end
